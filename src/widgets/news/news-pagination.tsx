...
const NewsPagination = () => {
    ...
    const pages = Array.from({ length: countPages }).map((_, i) => i + 1);
    ...
    if (countPages > maxPages) {
        visiblePages = pages.slice(
            currentPage > halfMaxPages ? (currentPage > countPages - halfMaxPages ? countPages - maxPages : startPage - halfMaxPages + 1) : 0,
            endPage
        );
    }
        ...
        <div className="flex items-center justify-center space-x-5">
            <Pagination className="min-h-12 rounded-[10px] bg-[#EBEBEB]">
                <PaginationContent className="space-x-5">
                    {countPages > maxPages && currentPage > halfMaxPages && (
                        <PaginationItem>
                            <PaginationEllipsis />
                        </PaginationItem>
                    )}
                    ...
                    {countPages > maxPages && currentPage <= countPages - halfMaxPages && (
                        <PaginationItem>
                            <PaginationEllipsis />
                        </PaginationItem>
                    )}
